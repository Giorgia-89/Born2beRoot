...
- choose strong root passwd
- create user (your login without 42)
- choose  passwd for user

----------
PARTITIONING THE DISK - bonus partudo apt install ufw  
...
---------------
ENCRYPTING THE DISK
-now the main partition has been set up. select configure encrypted volumes
-select yes then create encrypted volume.
...
- check that user was added to sudo group v
----------
CREATING A VM IN VIRTUALBOX
- launch virtualbox and click on new
- name Born2beRoot and store it in /sgoinfre/goinfre/Perso/your-login
...
- reboot
- log in again and check sudo superpowers via sudo -v
- from now on run root-privileged commands using sudo
- install apt: sudo apt-get update -y
- check for upgrades: sudo apt-get upgrade -y
...
- check status via sudo sudo apt install ufw  typing sudo vim /etc/ssh/sshd_config
...
specify which ports are enable to receive incoming connections (4242)
